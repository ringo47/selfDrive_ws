...
def line_fit(binary_warped, viz=None):
	"""
	...
	leftx_base = np.argmax(histogram[20:midpoint]) + 20
	...
	for window in range(nwindows):
		# Identify window boundaries/corners in x and y (and right and left)
...
def calc_curve(left_lane_inds, right_lane_inds, nonzerox, nonzeroy):
	...
	"""
	y_eval = yRes-1  # 720p video/image, so last (lowest on screen) y index is 719
...
def final_viz(undist, left_fit, right_fit, m_inv, left_curve, right_curve, vehicle_offset):
	...
	color_warp = np.zeros((yRes, xRes, 3), dtype='uint8')  # NOTE: Hard-coded image dimensions
	...
	label_str = 'Right lane radius:' + format(right_curve, ".2f") + 'm Left:' + format(left_curve, ".2f")
